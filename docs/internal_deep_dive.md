...
* Lines 29-33: PLL instance. `outclk_0` is `pix_clk`. Reset is
  ...
  active-low).
* Lines 73-87: sprite table active/shadow. Two 32×64 RAMs. Avalon
...
* Lines 94-118: cross `ctrl_swap_req` from `clk` into `pix_clk`. The
  ...
  2. The bulk assignment `for (int i=0; i<32; i++) sprite_table_active[i] <= ...`
     ...
     Quartus does this fine; it's still 32 × 64 = 2048 flops on one
...
* Lines 250-270: the `eval_done_r` sustain latch. The reason this
  ...
  the academic report §9.2. **Do not remove unless you replace it
...
## `hw/vga_timing.sv` — 800×525 frame walker

64 lines. The standard 640×480 @ 60 Hz parameters. Notes:
...
* Lines 240-244: background tile map address. `tile_col = x[9:3]`,
  ...
  the entire 80×60 grid.** scroll_x/scroll_y are routed in but
...
* Lines 195-265: main loop body. Read input → tick → render →
  commit → pace. Frame target is 16,666 µs (60 Hz). Skipping
...
* Lines 302-340: render_game_over. Overwrites the tile map with
  ...
  write/read mismatch (see §9.5 in the academic report).
...
* **Stripe rendering bug.** The Phase 2 build shows vertical
  ...
  tile-map byte-enable path (see academic report §9.5). The
...
§3.3 (eval) and the worst-case math in §3.3.3 (fetch) are the two
numbers most likely to come up. The "why the slave widens SWAP"
story in §3.6 / §9.3 is a great example of CDC discipline if asked
