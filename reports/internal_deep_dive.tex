...
\begin{itemize}[leftmargin=*]
  ...
  \item Lines 250--270: the \texttt{eval\_done\_r} sustain latch. The reason
    ...
    academic report Section 9.2. \textbf{Do not remove unless you replace it with
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Stripe rendering bug.} The Phase 2 build shows vertical
    ...
    tile-map byte-enable path (see academic report Section 9.5). The packed-byte
...
doc, walk them through it line by line. The cycle counts in Section 6 (eval) and
the worst-case math in Section 7 (fetch) are the two numbers most likely to come
up. The ``why the slave widens SWAP'' story in Section 4 / 9.3 of the academic
