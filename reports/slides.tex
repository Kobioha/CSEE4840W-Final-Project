...
\begin{tabular}{l}
\toprule
640$\times$480 @ 60\,Hz VGA on the DE1-SoC monitor \\
Tile-mapped battlefield (80$\times$60 from 64-tile ROM) \\
32 hardware sprites, top-8 sorted per scanline, hflip \\
256-entry RGB888 palette, M10K-inferred              \\
HUD strip: HP bar, AMMO, WAVE, ART, GAS, SCORE (BCD) \\
DragonRise SNES gamepad over evdev (\texttt{/dev/input/event0}) \\
60\,Hz HPS game loop driving sprite-table writes      \\
Frame-commit handshake (CTRL.SWAP $\to$ VSYNC bulk copy) \\
Mortar timer, artillery column kill + beam, mustard gas cloud \\
Wave spawner, level-up menu, ammo drops, game-over screen \\
\bottomrule
\end{tabular}
\end{frame}
